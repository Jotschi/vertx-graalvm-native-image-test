...
public class NetClientImpl implements MetricsProvider, NetClient {
	...
	public NetClient connect(int port, String host, String serverName, Handler<AsyncResult<NetSocket>> connectHandler) {
		doConnect(SocketAddress.inetSocketAddress(port, host), serverName,
			connectHandler != null ? ar -> connectHandler.handle(ar.map(s -> (NetSocket) s)) : null);
		return this;
	}
			...
			if (res.succeeded()) {
				...
				if (remainingAttempts > 0 || remainingAttempts == -1) {
					context.executeFromIO(() -> {
						...
						vertx.setTimer(options.getReconnectInterval(), tid -> doConnect(remoteAddress, serverName, connectHandler,
